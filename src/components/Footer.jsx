import { Link } from 'react-router-dom'

export default function Footer() {
  return (
    <footer className="bg-charcoal text-stone py-12 border-t border-stone/20">
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-3 gap-10">
        <div>
          <span className="font-display text-lg text-offwhite">Tlotleng Attorneys</span>
          <p className="font-body text-sm mt-2 max-w-xs">
            Full-service legal representation across South Africa.
          </p>
        </div>

        <div>
          <p className="font-body text-sm text-offwhite uppercase tracking-wide mb-3">
            Quick Links
          </p>
          <nav className="flex flex-col gap-2 font-body text-sm">
            <Link to="/" className="hover:text-ember transition-colors">Home</Link>
            <Link to="/about" className="hover:text-ember transition-colors">About</Link>
            <Link to="/services" className="hover:text-ember transition-colors">Services</Link>
            <Link to="/blog" className="hover:text-ember transition-colors">Blog</Link>
            <Link to="/contact" className="hover:text-ember transition-colors">Contact</Link>
          </nav>
        </div>

        <div>
          <p className="font-body text-sm text-offwhite uppercase tracking-wide mb-3">
            Practice Areas
          </p>
          <nav className="flex flex-col gap-2 font-body text-sm">
            <Link to="/services" className="hover:text-ember transition-colors">Criminal Law</Link>
            <Link to="/services" className="hover:text-ember transition-colors">RAF Claims</Link>
            <Link to="/services" className="hover:text-ember transition-colors">Family Law</Link>
          </nav>
        </div>
      </div>

      <div className="max-w-6xl mx-auto px-6 mt-10 pt-6 border-t border-stone/10 font-body text-sm">
        <p>&copy; {new Date().getFullYear()} Tlotleng Attorneys. All rights reserved.</p>
      </div>
    </footer>
  )
}